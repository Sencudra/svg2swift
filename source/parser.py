...
class Parser:
    ...
    def parse_svg(self, svg_description):
        ...
        flat_commands = [command for sublist in commands for command in sublist]
    ...
    def __split_move_to(self, string):
        if string[0] not in self.commands["move"]:
            raise ValueError("Wrong command passed")
        return [(string[0], command) for command in self.__find_digits_in_groups(string, group_size=2)]

    def __split_line_to(self, string):
        if string[0] not in self.commands["line"]:
            raise ValueError("Wrong command passed")
        group_size = 2 if string[0] in self.commands["line"][:2] else 1
        return [(string[0], command) for command in self.__find_digits_in_groups(string, group_size=group_size)]

    def __split_cubic_bezier_curve(self, string):
        if string[0] not in self.commands["cubic"]:
            raise ValueError("Wrong command passed")
        group_size = 6 if string[0] in self.commands["cubic"][:2] else 4
        return [(string[0], command) for command in self.__find_digits_in_groups(string, group_size=group_size)]

    def __split_quadratic_curve(self, string):
        if string[0] not in self.commands["quadratic"]:
            raise ValueError("Wrong command passed")
        group_size = 4 if string[0] in self.commands["quadratic"][:2] else 2
        return [(string[0], command) for command in self.__find_digits_in_groups(string, group_size=group_size)]

    def __split_elliptical_curve(self, string):
        if string[0] not in self.commands["elliptical"]:
            raise ValueError("Wrong command passed")
        group_size = 7
        return [(string[0], command) for command in self.__find_digits_in_groups(string, group_size=group_size)]
    ...
    def __split_command(self, string):
        ...
        elif command in self.commands["cubic"]:
            return self.__split_cubic_bezier_curve(string)
        elif command in self.commands["quadratic"]:
